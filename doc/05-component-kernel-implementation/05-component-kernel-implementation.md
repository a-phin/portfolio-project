...
The representation I'm selecting for the SongSelector component is Set. Set is an unordered data structure, similar to my component, and the component's kernel methods exactly map to SongSelector's five kernel methods. Because Set's kernel methods are similar to SongSelector's, it would be easy to implement SongSelector by using Set's kernel methods. The representation would restrict constant values to be greater than 0, and the representation would mirror Set's representation, so no additional modifications to the interpretation is not needed.
